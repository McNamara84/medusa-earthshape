class NestedResources::BibsController < ApplicationController
  ...
  def create
    ...
    respond_with @bib, location: adjust_url_by_requesting_tab(safe_referer_url), action: "error"
  end

  def update
    @bib = Bib.find(params[:id])
    @parent.bibs << @bib
    respond_with @bib, location: adjust_url_by_requesting_tab(safe_referer_url)
  end
